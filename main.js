...
function listMessages(auth, query){
  query = 'chinmaimoulya@gmail.com';
  return new Promise((resolve, reject) => {    
    const gmail = google.gmail({version: 'v1', auth});
    gmail.users.messages.list(      
      {        
        userId: 'me',  
        q:'chinmaimoulya@gmail.com is:unread',      
        maxResults:1     
      },  (err, res) => {        
                    if (err) {        
                        reject(err);          
    ...
    //console.log(res.data.labelIds.INBOX)
      if(!err){
        console.log("no error")
          ...
          var htmlBody = decode(body.replace(/-/g, '+').replace(/_/g, '/'));
          console.log(htmlBody)
          var docs = JSON.stringify(htmlBody);
          console.log(docs);
...
/*function whatsapp(params){
    fetch("https://api.callmebot.com/whatsapp.php?phone=+919490108199&text="+params+"&apikey=259207")
...
async function whatsapp(params) {
    const data1 = await fetch(
        "https://api.callmebot.com/whatsapp.php?phone=+919490108199&text="+params+"&apikey=259207"
    );
